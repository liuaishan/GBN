...
def train_op(model):    
    ...
    for epoch in range(args.epoch):
        ...
        for step, (train_x, train_label) in enumerate(train_loader):
            ...
            model.train()
            adv_x_L1,adv_x_L2,adv_x_Linf = adv_x_L1.cuda(),adv_x_L2.cuda(),adv_x_Linf.cuda()
            ...
            logits_Linf = model(adv_x_Linf)
            loss += loss_func(logits_Linf, clean_y)
            
            logits_L1 = model(adv_x_L1)
            loss += loss_func(logits_L1, clean_y)

            logits_L2 = model(adv_x_L2)
            loss += loss_func(logits_L2, clean_y)
            ...
            if step % 160 == 0:
                ...
                if testop_acc > best_testop_acc:
                    ...
                    torch.save(model.state_dict(), args.model_path + 'lenet5_AVG_best.pkl')
            if step % 40 == 0:
                with torch.no_grad():
        ...
        if epoch % 10 == 0:
            ...
            torch.save(model.state_dict(), args.model_path + 'lenet5_AVG' + str(epoch) + '.pkl')
                
    print('saving model...')
    if os.path.exists(args.model_path) == False:
        os.makedirs(args.model_path)

    torch.save(model.state_dict(), args.model_path + 'final_lenet5_AVG.pkl')
